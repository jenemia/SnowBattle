...
export class ThreeArena {
    ...
    buildScene() {
        ...
        const floor = new THREE.Mesh(new THREE.CircleGeometry(26, 80), new THREE.MeshStandardMaterial({
            color: "#d9f6ff",
            metalness: 0.08,
            roughness: 0.82
        }));
        ...
        const ring = new THREE.Mesh(new THREE.TorusGeometry(26, 0.24, 12, 120), new THREE.MeshBasicMaterial({ color: "#8ee4ff" }));
    ...
    syncProjectiles(projectiles) {
        ...
        for (const projectile of projectiles) {
            ...
            if (!mesh) {
                mesh = new THREE.Mesh(new THREE.SphereGeometry(projectile.radius, 16, 16), new THREE.MeshStandardMaterial({
    ...
    createPlayerMesh(player) {
        ...
        const body = new THREE.Mesh(new THREE.CylinderGeometry(0.95, 1.1, 1.2, 16), new THREE.MeshStandardMaterial({
        ...
        group.add(body);
        const head = new THREE.Mesh(new THREE.SphereGeometry(0.38, 18, 18), new THREE.MeshStandardMaterial({ color: "#f4ffff" }));
        head.position.set(0, 0.95, 0);
        head.castShadow = true;
        group.add(head);
        const nose = new THREE.Mesh(new THREE.ConeGeometry(0.16, 0.4, 8), new THREE.MeshStandardMaterial({ color: "#ffb28c" }));
    ...
    render() {
        ...
        if (state) {
            const focusPlayer = state.players.find((player) => player.sessionId === this.localSessionId) ??
                state.players[0];
            if (focusPlayer) {
                const idealPosition = new THREE.Vector3(focusPlayer.x, 22, focusPlayer.y + 15);
