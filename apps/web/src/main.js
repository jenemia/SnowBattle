...
const requeueButton = document.querySelector("#requeue-button");
...
const playerAState = document.querySelector("#player-a-state");
const playerBName = document.querySelector("#player-b-name");
const playerBState = document.querySelector("#player-b-state");
if (!viewport ||
    !guestNameInput ||
    !saveNameButton ||
    !requeueButton ||
...
const client = new SnowBattleClient(import.meta.env.VITE_SERVER_URL || "ws://localhost:2567", {
    onCountdown(message) {
    ...
    onQueueStatus(message) {
        ui.queueCount.textContent = `${message.queuedPlayers} rider${message.queuedPlayers === 1 ? "" : "s"}`;
...
ui.saveNameButton.addEventListener("click", () => {
    localStorage.setItem("snowbattle.guestName", ui.guestNameInput.value.trim() || storedName);
...
function currentMovementVector() {
    const x = Number(movement.has("d") || movement.has("ArrowRight")) -
        Number(movement.has("a") || movement.has("ArrowLeft"));
    const y = Number(movement.has("s") || movement.has("ArrowDown")) -
